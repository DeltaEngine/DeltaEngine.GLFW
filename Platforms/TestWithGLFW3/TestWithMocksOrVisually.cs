...
{
	/// <summary>
	/// Automatically tests with MockResolver when NCrunch is used, otherwise GLFW3 is used.
	/// </summary>
	[TestFixture]
	public class TestWithMocksOrVisually
	{
			...
			float timeToAddInSeconds = 1.0f / Settings.DefaultUpdatesPerSecond)
		{
			...
			if (CheckIfWeNeedToRunTickToAvoidInitializationDelay())
				RunTickOnce(renderer, drawing);//ncrunch: no coverage
			var startTimeMs = GlobalTime.Current.Milliseconds;
			do
				RunTickOnce(renderer, drawing);
			while (GlobalTime.Current.Milliseconds - startTimeMs + MathExtensions.Epsilon <
				timeToAddInSeconds * 1000);
		}
